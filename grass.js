...
void main() {
  ...
  col = mix(vec4(0.0, 0.9, 0.0, 1.0), col, frc);
  gl_FragColor = col;
}`;
...
var min = -0.25;
var max =  0.25;
var vertices = sourceMesh.geometry.vertices
sourceMesh.geometry.vertices.forEach(v => {
    v.x += THREE.Math.randFloatSpread(2);
   
    v.z += THREE.Math.randFloatSpread(2);
  });
sourceMesh.geometry.computeFaceNormals();
//For each instance of the grass blade
for (var i = 0; i < instances; i++){
  //Offset of the roots

		if(sourceMesh.geometry.faces.length < instances) instances = sourceMesh.geometry.faces.length;
	
	if(sourceMesh.geometry.faces[i] != undefined) 
	face = sourceMesh.geometry.faces[i]
	var vector = ( vertices[ face.a ] )
			.add( vertices[ face.b ] )
			.add( vertices[ face.c ] )
			.divideScalar( 3 )
...
createGrass.prototype.update = function(dt){
